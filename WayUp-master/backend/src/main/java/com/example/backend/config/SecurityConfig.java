...
public class SecurityConfig {
    ...
    private static final String[] PUBLIC_ENDPOINTS = { "/register", "/login", "/test/**" ,"/api/1/suggested-companies","/api/debug/list-models"};//"/api/internships/**","/api/internships/me";
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        // Permet toutes les origines pendant le développement
        configuration.setAllowedOriginPatterns(Arrays.asList("*"));  // ou configuration.setAllowedOrigins(Arrays.asList("*"));
        configuration.setAllowedMethods(Arrays.asList("GET", "POST", "PUT", "DELETE", "OPTIONS"));
        configuration.setAllowedHeaders(Arrays.asList("Authorization", "Content-Type"));
        configuration.setAllowCredentials(true);  // Permet l'envoi de cookies avec les requêtes
    
        UrlBasedCorsConfigurationSource source = new UrlBasedCorsConfigurationSource();
        source.registerCorsConfiguration("/**", configuration);  // Applique cette config CORS à toutes les routes
        return source;
    }
  }